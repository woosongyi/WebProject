...
<body>
	...
	<c:if test="${not empty requestScope.tourinfo }">
		<table border="1" width="1200">
			...
			<tr>
				<th><img src="${requestScope.tourinfo.firstimage }" width=150
					height=150 /></th>
				<th>${requestScope.tourinfo.title }</th>
				<th>${requestScope.tourinfo.address }<br>
					${requestScope.tourinfo.address2 }
				</th>
				<th>전화명 : ${requestScope.tourinfo.telname }<br> 전화번호 :
					${requestScope.tourinfo.tel }<br> 홈페이지 :
					${requestScope.tourinfo.homepage }
				</th>
				<th>${requestScope.tourinfo.overview }</th>
			</tr>
		</table>
